...
{
    public partial class AylikKiyasForm : Form
    {
        ...
        public AylikKiyasForm(KaloriUygulamaDbContext db, Kullanici kullanici)
        {
            this.db = db;
            this.kullanici = kullanici;
            InitializeComponent();
            dtpBitis.Value = dtpBaslangic.Value.AddDays(29);//30günlük
            baslangicTarih = dtpBaslangic.Value;
            bitisTarih = dtpBitis.Value;
            AylikBesinDegerleriRaporu();
        }

        private void AylikBesinDegerleriRaporu()
        {
            if (kullanici.Ogunler.Any())
            {
                ...
                lblAylikKalori.Text = kalori;
                lblAylikProtein.Text = protein;
                lblAylikKarbonhidrat.Text = karbonhidrat;
                lblAylikYag.Text = yağ;

                // (Ekstra) Seçili haftanın günlük ortalama besin değerleri
                var günlükOrtalamaKalori = decimal.Parse(kalori) / 30;
                var günlükOrtalamaProtein = decimal.Parse(protein) / 30;
                var günlükOrtalamaKarbonhidrat = decimal.Parse(karbonhidrat) / 30;
                var günlükOrtalamaYag = decimal.Parse(yağ) / 30;
        ...
        {
            dtpBitis.Value = dtpBaslangic.Value.AddDays(29);
            baslangicTarih = dtpBaslangic.Value;
            bitisTarih = dtpBitis.Value;
            AylikBesinDegerleriRaporu();
        }

        private void btnAylikKiyasCikis_Click(object sender, EventArgs e)
        {
            Close();
        }
    }
